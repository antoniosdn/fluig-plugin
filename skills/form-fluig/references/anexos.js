...
function anexo(event) {
  try {
    ...
    const inputFile = $(event.currentTarget)
      ...
      .parent()
      .find(".inputAnexo")[0];
    const fileDescription = $(event.currentTarget)
      .parent()
      .parent()
      .find(".descAnexo")
      .val();
    ...
    if (!getMobile()) {
      ...
      if (tabAttachments) {
        ...
        if (parent.WCMAPI.isIe9()) {
          $(".ecm-navigation-silverlight", parent.document)
            .show("fade")
            .css("top", 0);
          $("#ecm-navigation-silverlight", parent.document).attr({
            "data-on-camera": "true",
            "data-file-name-camera": fileDescription,
            "data-inputNameFile": idInput,
          });
          $(parent.document).on("keyup", this.actionKeyup);
        } else {
          var element = parent.document.getElementById(
            "ecm-navigation-inputFile-clone",
          );
  ...
  try {
    if (hasFileFluig(fileDescription)) {
      ...
      for (let i = 0; i < anexos.length; i++) {
        ...
        if (fileDescription == descricao) {
          parent.WKFViewAttachment.openAttachmentView(
            "adm",
            anexos[i].documentId,
          );
          return;
...
function downloadFile(fileDescription, idInput) {
  try {
    const filename =
      getMode() == "VIEW" ? $(`#${idInput}`).text() : $(`#${idInput}`).val();
...
function btnState(idInput, acao, btn) {
  try {
    ...
    let btnDownloadFile = $(`#${idInput}`)
      .parent()
      .parent()
      .find(".btnDownloadFile");
    let btnViewerFile = $(`#${idInput}`)
      .parent()
      .parent()
      .find(".btnViewerFile");
    if (acao == "delete") {
      ...
      btnUpFile
        .find("i")
        .removeClass("fluigicon-file-upload")
        .addClass("fluigicon-trash");
      if (btn == "download") {
    ...
    if (acao == "upload") {
      ...
      btnUpFile
        .find("i")
        .removeClass("fluigicon-trash")
        .addClass("fluigicon-file-upload");
...
function invalidFilesTable(tablename, idInput) {
  try {
    let errors = "";
    const countRows = $(`[tablename='${tablename}']`)
      .find("tbody tr")
      .not(":first");
    for (let i = 0; i < countRows.length; i++) {
      let indice = getIndice(countRows.eq(i).find("input")[0].id);
      let inputNameFile = $(`#_${idInput}___${indice}`).length
        ? $(`#_${idInput}___${indice}`)
        : $(`#${idInput}___${indice}`);
...
function invalidFile(idInput) {
  try {
    ...
    if (inputNameFile) {
      if ($(`#_${idInput}`).length) {
        let fileDescription = $(`#_${idInput}`)
          .parent()
          .find(".descAnexo")
          .val();
        return !hasFileFluig(fileDescription);
      } else {
        let fileDescription = $(`#${idInput}`)
          .parent()
          .find(".descAnexo")
          .val();
        return !hasFileFluig(fileDescription);
 ...
 */
function getMobile() {
  return parent.WCMAPI.isMobileAppMode();
}

/**
 * Retorna o modo de visualização do formulário:
 * ADD (Adição), MOD (Edição) ou VIEW (Visualização)
 */
function getMode() {
  if (parent.ECM && parent.ECM.workflowView) {
    return parent.ECM.workflowView.state;
  }
  return "ADD"; // Padrão caso não consiga identificar
}

/**
 * Retorna o índice de campos Pai e Filho (se necessário)
 */
